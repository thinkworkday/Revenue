...
//Cron Job Monarch Digital Perion function
async function digitalPerionStatCronJob() {
    console.log('Monarch Digital Perion Cron Job start!');
    var startDate = moment().utc().subtract(2, "days").format("YYYY-MM-DD");
    var endDate = moment().utc().subtract(2, "days").format("YYYY-MM-DD");

    //GET Monarch Digital Perion Data From Company
    let companyAql = `FOR company in companies FILTER company.name == "Monarch Digital" FOR provider IN company.reportingProviders FILTER provider.reportingProvider == "perion" RETURN company`
    try {
    ...
    console.log('Monarch Digital Perion Cron Job End!');
}
