...
async function verizonDirectBeforeCronJob() {

    var requestDate = moment.utc().subtract(2, "days").format("YYYYMMDD");
...
function saveVerizonStat(verizonData) {
    ...
    try {
        db.query(aql`FOR doc IN ${verizonList} UPSERT { date: doc.date, subid: doc.subid } INSERT doc UPDATE {date: doc.date, sourceTag: doc.sourceTag, userCountry: doc.userCountry, searches: doc.searches, biddedSearches: doc.biddedSearches, biddedResults: doc.biddedResults, biddedClicks: doc.biddedClicks, revenue: doc.revenue, coverage: doc.coverage, ctr: doc.ctr, cpc: doc.cpc, tqScore: doc.tqScore, rn: doc.rn } IN verizon_direct_reports`);
    } catch (error) {
        console.log(error);
    }
    console.log("Verizon Direct Before Day Update Stat Cron Job End!");
}

module.exports = {
    verizonDirectBeforeCronJob,
}