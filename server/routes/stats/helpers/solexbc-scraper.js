...
//Get SolexBC Stat
async function getSolexBCStat(company, start, end) {
    ...
    return new Promise((resolve, reject) => {
        try {
            db.query(`FOR doc IN solexbc_stat_reports FILTER doc.date >= ${startDate} && doc.date <= ${endDate} COLLECT date = doc.date, subid = doc.subid AGGREGATE revenue = SUM(TO_NUMBER(doc.revenue)), searches = SUM(TO_NUMBER(doc.searches)), clicks = SUM(TO_NUMBER(doc.clicks)), split = AVERAGE(TO_NUMBER(doc.split)) SORT date DESC RETURN {date, subid, revenue, searches, clicks, split, profit: (100-split)*revenue/100, publisherNet: revenue*split/100 }`)
                .then(cursor => {
                    return cursor.map(cur => {
                        return cur;
                    })
...
//Get Solexbc Stat for publisher
async function getPublisherSolexbcStat(company, start, end) {
    startDate = moment.utc(start, "MM-DD-YYYY").startOf('day').toDate().getTime();
    endDate = moment.utc(end, "MM-DD-YYYY").endOf('day').toDate().getTime();

    return new Promise((resolve, reject) => {
        try {
            db.query(`FOR doc IN solexbc_stat_reports FILTER doc.date >= ${startDate} && doc.date <= ${endDate} FOR tag IN tags FILTER tag.advertiser == "solex-bc" FOR ts IN tag.subids FILTER (ts.filterTag == 'StartsWith' && STARTS_WITH(doc.subid, ts.subid)) || ts.filterTag == 'EndsWith' && LIKE(doc.subid, ts.subid + '%') || (ts.filterTag == 'Contains' && CONTAINS(doc.subid, ts.subid)) || (ts.filterTag == 'ExactValue' && doc.subid == ts.subid) COLLECT date = doc.date, subid = doc.subid AGGREGATE revenue = SUM(TO_NUMBER(doc.revenue)), searches = SUM(TO_NUMBER(doc.searches)), clicks = SUM(TO_NUMBER(doc.clicks)), split = AVERAGE(TO_NUMBER(doc.split)) SORT date, TO_NUMBER(subid) DESC RETURN {date, revenue: revenue*split/100, subid, clicks, searches, ctr: clicks/searches, cpc: revenue*split/100/clicks }`)
                .then(cursor => {
...
async function getChartMetrics(company, startDate, endDate) {
    ...
    if (companyInfo) {
        console.log("************** SolexBC Chart Metrics **************")
        for (var reportingProvider of companyInfo.reportingProviders) {
            if (reportingProvider.reportingProvider == "solex-bc") {
                return new Promise((resolve, reject) => {
                    try {
                        db.query(`
                                LET revenuePerDay = ( // subquery start
                                    FOR r IN solexbc_stat_reports
